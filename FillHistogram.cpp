...
int main(int argc, char *argv[])
{
   ...
   TH1D HJetPToRefPT("HJetPToRefPT", "|eta| < 1.6, Ref PT > 30", 100, 0, 2.5);
   TH1D HRawPToRefPT("HRawPToRefPT", "|eta| < 1.6, Ref PT > 30", 100, 0, 2.5);
   ...
   TH1D HEvtRho_avg("HEvtRho_avg", "|eta| < 1.6", 100, 0, 300);
   TH1D HEvtRho_avg_cent("HEvtRho_avg_cent", "|eta| < 1.6, 0 < Centrality < 10%", 100, 0, 300);
   ...
   vector<float> *rawpt = nullptr;

   vector<double> *etaMin = nullptr;
   vector<double> *etaMax = nullptr;
   vector<double> *evtRho = nullptr;
   ...
   Tree->SetBranchAddress("evtRho", &evtRho);
   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      int rhosize = evtRho->size();
      int rhoii = 0;
      int rhocount=0;
      double rhoavg=0;

      while((*etaMin)[rhoii]<1.6&&rhoii<rhosize){
         if((*etaMax)[rhoii]>-1.6&&(*etaMin)[rhoii]<1.6){
            rhoavg+=(*evtRho)[rhoii];
            rhocount++;
         }
         rhoii++;
